...
class LocalGateway extends \WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->title = (string) $this->get_option('title', __('Mobile Banking', 'astimpay-gateway'));
        $this->description = (string) $this->get_option('description', __('Pay securely via Bangladeshi payment methods.', 'astimpay-gateway'));
    ...
    protected function setup_properties()
    {
        ...
        $this->method_title = __('AstimPay', 'astimpay-gateway');
        $this->method_description = sprintf(
            '%s<br/><a href="%s" target="_blank">%s</a>',
            __('Accept Bangladeshi payments via multiple gateways including bKash, Nagad, Rocket and more.', 'astimpay-gateway'),
    ...
    protected function is_valid_for_use()
    {
        if (empty($this->api_key) || empty($this->api_url)) {
            $this->add_error(__('AstimPay requires API Key and API URL to be configured.', 'astimpay-gateway'));
            return false;
        }
        return true;
    }
    ...
    public function init_form_fields()
    {
        ...
        $base_fields = [
            'enabled' => [
                ...
                'type' => 'checkbox',
                'label' => __('Enable AstimPay', 'astimpay-gateway'),
                'default' => 'no',
            ],
            'title' => [
                ...
                'default' => __('Bangladeshi Payment', 'astimpay-gateway'),
                'desc_tip' => true,
            ],
            'description' => [
                ...
                'default' => __('Pay securely via Bangladeshi payment methods.', 'astimpay-gateway'),
                'desc_tip' => true,
            ],
            'api_key' => [
        ...
        if ($currency !== 'BDT') {
            $base_fields['exchange_rate'] = [
                ...
                'type' => 'text',
                'desc_tip' => true,
                'description' => __('This rate will be applied to convert the total amount to BDT', 'astimpay-gateway'),
                'default' => '0',
                'custom_attributes' => [
                    'step' => '0.01',
                    'min' => '0',
        ...
        $base_fields['debug'] = [
            ...
            'description' => sprintf(
                ...
                '<code>' . \WC_Log_Handler_File::get_log_file_path('astimpay') . '</code>'
            ),
        ];
    ...
    public function process_payment($order_id)
    {
        try {
            ...
            $order->update_status(
                OrderStatus::PENDING,
                __('Awaiting AstimPay payment', 'astimpay-gateway')
            );
    ...
    public function handle_webhook()
    {
        try {
            ...
            wp_die($e->getMessage(), 'AstimPay Webhook Error', ['response' => 500]);
        }
    }
    ...
    protected function process_order_status($order, $data)
    {
        ...
        $order->update_meta_data('astimpay_payment_data', $data);
        ...
        if ($data->status === 'COMPLETED') {
            ...
            $order->update_status(
                OrderStatus::ON_HOLD,
                __('Payment is on hold. Please check manually.', 'astimpay-gateway')
            );
        }
    ...
    protected function handle_completed_payment($order, $data)
    {
        ...
        $note = sprintf(
            __('Payment via %s. Amount: %s, Transaction ID: %s', 'astimpay-gateway'),
            $data->payment_method,
            $data->amount,
            $data->transaction_id
        );
    ...
    public function display_transaction_data($order)
    {
        ...
        if (defined('ASTIMPAY_ADMIN_DATA_DISPLAYED')) {
            return;
        }
        ...
        $payment_data = $order->get_meta('astimpay_payment_data');
        ...
        // Mark as displayed
        define('ASTIMPAY_ADMIN_DATA_DISPLAYED', true);
    }
    ...
    protected function display_payment_info_html($data)
    {
        ...
        echo "<div class='form-field form-field-wide astimpay-admin-data'>

            <table class='wp-list-table widefat striped posts'>
                <tbody>
                    <tr>
                        <th>
                            <strong>Payment Method ({$this->title})</strong>
                        </th>
                        <td>
                                {$payment_method}
                        </td>
