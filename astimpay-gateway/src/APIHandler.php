...
class APIHandler
{
    ...
    private static function log($message, $level = 'debug')
    {
        if (self::$debug) {
            ...
            wc_get_logger()->log($level, $message, ['source' => 'astimpay']);
        }
    }
     ...
     *
     * @param string $invoice_id Invoice ID to Query
     * @return object Response object
     */
    public static function query_payment($invoice_id)
    {
        try {
            if (empty($invoice_id)) {
                throw new \Exception('Invoice ID is required');
